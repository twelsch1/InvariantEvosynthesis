...
public class Reduction {
	 ...
	 * because of restrictions from localRestrictions that will be relaxed during RBPS algorithm. As an example, 
	 * consider we are synthesizing a predicate that is true where x is gte y and z.
	 * Let an extraAssertion on the input space be <strong>(and (>= x 7) (>= x y))</strong> i.e. x must be gte 7 and y for any CounterExample.
	 * Under this input space, we find the following predicate, <strong>(and (>= x z) (>= x 3))</strong>. This predicate is equivalent on this 
	 * restricted input space, as we now have covered the scenario where x is greater than y and z. Further, x must be gte 7, so x gte 3 is a tautology. 
	 * However,
	 ...
	 * <strong>(and (>= x z) (>= x 3))</strong>. A correct solution here could be <strong>(>= x y)</strong>. This would then conclude the RBPS process,
	 * and the predicate would be be <strong>(and (and (>= x z) (>= x 3)) (>= x y))</strong>. But for this predicate there exists a CounterExample where x is less
	 * than 3 but x is still greater than z and y, so this is not correct. This function is called to avoid this issue. Moving back
	 * to the original success of <strong>(and (>= x z) (>= x 3))</strong>, we would run this function and modify it to be
	 * <strong>(and (>= x z) true)</strong>. We then have this as a restriction and synthesize <strong>(>= x y)</strong> concluding RBPS, but 
	 * this time the predicate is <strong>(and (and (>= x z) true) (>= x y))</strong> which is correct.

	 * @param verifier A Verifier class instance used for verification.
...
//		////System.out.println("Removing cons and tauts");
		...
		//We will eventually need this to additionally include the Bool variables passed into the function as parameters.
